...
public class ConnectTimeoutSettingActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String mqttConfigAppStr = SPUtiles.getStringValue(ConnectTimeoutSettingActivity.this, AppConstants.SP_KEY_MQTT_CONFIG_APP, "");
    ...
    public void onMQTTMessageArrivedEvent(MQTTMessageArrivedEvent event) {
        ...
        if (msgCommon.msg_id == MQTTConstants.NOTIFY_MSG_ID_CONNECT_TIMEOUT) {
    ...
    public void onMQTTPublishSuccessEvent(MQTTPublishSuccessEvent event) {
        ...
        if (msgId == MQTTConstants.CONFIG_MSG_ID_CONNECT_TIMEOUT) {
    ...
    private void getConnectTimeout() {
        XLog.i("读取服务器重连超时时间");
        String appTopic;
        ...
        String message = MQTTMessageAssembler.assembleReadConnectTimeout(mMokoDevice.uniqueId);
        try {
            MQTTSupport.getInstance().publish(appTopic, message, MQTTConstants.READ_MSG_ID_CONNECT_TIMEOUT, appMqttConfig.qos);
    ...
    public void onSave(View view) {
        ...
        showLoadingProgressDialog();
        setReportInterval(timeout);
    }

    private void setReportInterval(int timeout) {
        XLog.i("设置服务器重连超时时间");
        ConnectionTimeout data = new ConnectionTimeout();
        ...
        String message = MQTTMessageAssembler.assembleConfigConnectTimeout(mMokoDevice.uniqueId, data);
        try {
            MQTTSupport.getInstance().publish(appTopic, message, MQTTConstants.CONFIG_MSG_ID_CONNECT_TIMEOUT, appMqttConfig.qos);
        } catch (MqttException e) {
            e.printStackTrace();
        }
    }
